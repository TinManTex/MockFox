...
function this.RuntimeDumps()
  ...
  local varsTable=this.DumpVars(vars)

  local cvarsTable=this.DumpVars(cvars)
  ...
  this.WriteTable(this.dumpDir.."\\varsDump\\".."cvars.lua",table.concat(header,"\r\n"),cvarsTable)
  ...
  local header={
    [[--mvars.lua]],
    [[--using Inspect, so is designed to be human readable rather than lua loadable]],
    [[--dump of vars during missionCode:]]..vars.missionCode,
  }
...
function this.DumpVars(inputVars)
