...
this.runtimeVarsOutPath=this.dumpDir.."\\varsDump\\"
--DEBUGNOW this.runTimeVarsOutPath=InfCore.paths.vscode
...
function this.Save()
  if Ivars.debug_WriteRuntimeVarsOnSave:Is(1)then
    this.WriteRuntimeVars()
  end
end--Save
...
function this.GenerateMockModules(options)
  ...
  this.WriteTable(this.dumpDir.."MockModulesGenerated.lua",table.concat(header,"\r\n"),mockModules)
  ...
  if this.writeDebugOutput then
    ...
    this.WriteTable(outDir.."exeModuleRefsEntryOrder.lua",table.concat(header,"\r\n"),exeModuleRefsEntryOrder)
...
--TODO: dump raw _G
function this.DumpRuntimeVars(outPath)
  ...
  for varsName,varsTable in pairs(varsTables)do
    ...
    local filePath=outPath.."\\"..varsName..".lua"
    this.WriteTable(filePath,table.concat(header,"\r\n"),varsTable)
  end--for varsTable
  --< snapshot
  ...
  for varsName,varsTable in pairs(varsTables)do
    ...
    table.insert(header,[[--dump of vars during missionCode:]]..vars.missionCode)
    local outPath=this.dumpDir.."\\varsAsGlobal\\"..varsName..".lua"
...
function this.DoTearDown()
  ...
  this.DumpRuntimeVars(this.runtimeVarsOutPath)
  if this.writeDebugOutput then
...
function this.WriteRuntimeVars()
  this.DumpRuntimeVars(this.runTimeVarsOutPath)
end--WriteRuntimeVars

this.debug_WriteRuntimeVarsOnSave={
  inMission=true,
...
this.langStrings={
  ...
  help={
    eng={
      debug_WriteRuntimeVarsOnSave="Runs WriteRuntimeVars on IH save (which includes close IH menu)",
      WriteRuntimeVars="Writes vars, svars, gvars, mvars to GameDir/mod/vscode, for manual perusal or use with vscode",
    },
  },
}--langStrings
