...
--tex mostly for input /Info/ files
this.mockFoxPath=[[d:\github\MockFox\]]

this.dumpDir=this.mockFoxPath..[[\Generated\tpp\]]--tex general output folder
--also outputs to:
--<dumpDir>/varsDump -DumpRuntimeVars - for reference, IH also has Debug Menu > vscode menu > WriteVars
--<dumpDir>/globalsDump
...
--tex unmodded lua, all in same folder, only for buildFromScratch=true, don't need if buildFromScratch=false(the default) which uses the built IHGenModuleReferences
this.luaPath=[[E:\GameData\mgs\filetype-crush\lua\]]

this.classesPath=this.mockFoxPath..[[Info\LuaClasses[sais ida dump]_sorted.txt]]
this.exeCreateModulesLogPath=this.mockFoxPath..[[Info\log_createmodule.txt]]
...
function this.GenerateMockModules(options)
  ...
  for tableName,globalTable in pairs(globalsByType)do
    ...
    InfCore.WriteTable(outDir..fileName,header,globalTable)
  ...
  InfCore.WriteTable(this.dumpDir.."IHGenModuleReferences.lua",header,luaSourceRefs)
  end
  ...
  if setThisAsGlobal then
    outDir=this.dumpDir..[[mockModulesAsGlobal\]]
  end
  this.WriteMockModules(outDir,mockModules,exeModuleRefs,exeModuleRefsEntryOrder,setThisAsGlobal)
  InfCore.WriteTable(this.dumpDir.."MockModulesGenerated.lua",header,mockModules)
  ...
  if this.writeDebugOutput then
    ...
    InfCore.WriteTable(outDir.."exeModuleRefs.lua",header,exeModuleRefs)

    local header={
      [[--exeModuleRefsEntryOrder.lua]],
      [[--GENERATED by IHTearDown BuildModuleRefsFromExeLog]],
      [[--counterpart to exeModuleRefs]],
      [[--order of modules according to hooking/logging the exe module creation functions]],
      [[--just for debugging GenerateMockModules process]]
    }
    InfCore.WriteTable(outDir.."exeModuleRefsEntryOrder.lua",header,exeModuleRefsEntryOrder)
    ...
    InfCore.WriteTable(outDir.."exeRefsNotFoundInLive.lua",header,exeRefsNotFoundInLive)
    ...
    InfCore.WriteTable(outDir.."liveNotFoundInExeRefs.lua",header,liveNotFoundInExeRefs)
    ...
    InfCore.WriteTable(outDir.."luaRefsNotFoundInLive.lua",header,luaRefsNotFoundInLive)
    ...
    InfCore.WriteTable(outDir.."liveNotFoundInLuaRefs.lua",header,liveNotFoundInLuaRefs)
    ...
    InfCore.WriteTable(outDir.."IHMissedModules.lua",header,missedModules)
    ...
    InfCore.WriteTable(outDir.."IHLiveModuleKeysVsMock.lua",header,liveModuleKeysVsMock)
  end--if this.writeDebugOutput 
  --write GenerateMockModules process debugging stuff<
end--GenerateMockModules

--GOTCHA: IH currently has Debug menu > vscode menu > WriteVars, this is just for reference till I can figure out a nice way to get them to play nice together
...
function this.DumpRuntimeVarsDebug()
  ...
  InfCore.WriteTable(this.dumpDir.."\\debugDump\\"..svarsDeclaredName,header,svarsDeclaredTable)
  ...
  InfCore.WriteTable(this.dumpDir.."\\debugDump\\"..gvarsDeclaredName,header,gvarsDeclaredTable)
...
function this.OtherRunTimeAnalyze()
  ...
  InfCore.WriteTable(this.dumpDir.."\\misc\\".."IHGenUnfoundLiveClassesIda.lua",header,nonLiveClasses)
  ...
  InfCore.WriteTable(this.dumpDir.."\\misc\\".."IHGenEntityClassDictionary.lua",header,entityClassDictionary)
...
function this.BuildMockModulesFromReferences(liveModules,moduleReferences)
  ...
  --                end
              ...
              InfCore.Log("WARNING: BuildMockModulesFromReferences "..referenceModuleName.."["..tostring(referenceKey).."] unknown key type "..type(referenceKey))
            end--if livevalue
...
function this.DumpToFiles(outDir,moduleTable)
  ...
  for k,v in pairs(moduleTable) do
    ...
    local ins=Inspect(v)
    InfCore.WriteTable(filename,nil,v)
  end
end
...
--setThisAsGlobal: module will be written as <moduleName>={, therefore defining itself as global when its loaded, useful for using as vscode extensio lua language server workspace library feature
...
function this.WriteMockModules(outDir,mockModules,exeModuleRefs,exeModuleRefsEntryOrder,setThisAsGlobal)
  ...
  for moduleName,mockModule in pairs(mockModules) do
    ...
    InfCore.WriteStringTable(filename,lines)
  end--for mockModules
end--WriteMockModules

function this.CreatePathForFile(filePath)
  if not InfCore.FileExists(filePath)then
    local lastSlashIdx=filePath:match'^.*()/'
    local path=filePath:sub(1,lastSlashIdx)
    local cmd=[[cmd.exe /c mkdir "]]..path..[["]]
    InfCore.Log(cmd)

    InfCore.PCall(function()os.execute(cmd)end)
  end
end--CreatePathForFile
