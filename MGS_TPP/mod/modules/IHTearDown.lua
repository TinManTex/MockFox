...
--  },
  ...
  local liveModuleKeysVsMock=this.CheckFoxTableKeysAccountedFor(globalsByType.table,mockModules)
  if this.debugModule then
  ...
  local header={
    ...
    [[--GENERATED by IHTearDown BuildModulesFromExeLog]],
  ...
  local header={
    ...
    [[--references from exeModules (from BuildModulesFromExeLog) that werent found in live session]],
    [[--just for debugging DumpModules process]]
  }
  ...
  this.WriteTable(this.dumpDir.."IHLiveModuleKeysVsMock.lua",table.concat(header,"\r\n"),liveModuleKeysVsMock) 
...
function this.RuntimeDumps()
  ...
  local header={
    ...
    [[--GOTCHA: runtime vars arrays are indexed from 0, output is indexed from 1 lua style]],
